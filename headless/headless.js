var page = require('webpage').create();
    ...
    page.masterTimeout = 600000; // 10min
...
function exit (msg) {
  ...
  console.log(msg);
  console.log('used ' + colored("blue", Math.round(bandwidth / 1024)) + ' KBs');
  console.log('closing ...');

  try { fs.write(fs.file, ' \n ### END OF SESSION ### \n\n' , 'a'); } 
...
// close phantom if the program exceeds the master timeout
setTimeout(function (){

  exit('exceeded master timeout!');

}, page.masterTimeout); 

if (!usr || !pswd) {
  
  exit('missing some arguments!');

}

// writes the msg to the user log
console.debug = function (msg) {

  var d = new Date(),
      // DD/MM/YYYY
      today = [d.getDate(), d.getMonth() + 1, d.getFullYear()].join('/'),
      // HH:MM:SS
      time = [d.getHours(), d.getMinutes(), d.getSeconds()].join(':'),
      // DD/MM/YYYY HH:MM:SS : <MSG>
      prettymsg = today + ' ' + time + ' : ' + msg;

  try { fs.write(fs.file, prettymsg + '\n' , 'a'); } 
  catch(e) { console.log(e); }

};
// replace the default logging method with sys.out (for colors)
console.log = function (msg) {

  sys.stdout.write(userColor + ' ' + msg + '\n');
  console.debug(msg);

};

// apply unix color coding to a string
colored = function(color, msg) {

  var os = require('system').os,
      colors = {
        "rainbow": "\033[9" + (Math.round(Math.random() * 8) + 1) + "m",
        "pink":   '\033[95m',
        "blue":   '\033[94m',
        "green":  '\033[92m',
        "red":    '\033[93m',
  ...
  if ( !colors[color]) {
    ...
    console.debug('no such color!');
    return msg;

  }

  return colors[color] + msg + endColor;

};
var userColor = colored("rainbow", usr);

page.onInitialized = function () {

  // the application is relying on a SWFObject check and we do not have flash.
  // so we'll need to inject a function to circumvent this check.
  page.evaluate(function () {
    
    var injectSWF = setInterval(function () {
    
      try {
      
        swfobject.hasFlashPlayerVersion = function () { return true; };
        clearInterval(injectSWF);
...
page.onError = function (msg, trace) {
  ...
  console.debug(msgStack.join('\n'));

};

// log alert occurrences 
page.onAlert = function (msg) { console.debug('ALERT: ' + msg); };

// log url changes (navigation)
page.onUrlChanged = function(targetUrl) {
  
  console.debug('Changed URL: ' + targetUrl);

};

// intercept unwanted / unnecessary files 
page.onResourceRequested = function (requestData, networkRequest) {

    if (
      requestData.url.indexOf('.png') !== -1 || 
      requestData.url.indexOf('.jpg') !== -1 || 
      requestData.url.indexOf('dc.js') !== -1 || 
      requestData.url.indexOf('.woff') !== -1 || 
      requestData.url.indexOf('.css') !== -1
    ) {
      // ensure that images aren't loaded
      networkRequest.abort();
    }

};

// log loaded resources, incriminate bandwidth counter
page.onResourceReceived = function(resp) {

  if (resp.stage === 'end' && resp.url !== '') {

    console.debug('Fetched (#' + resp.id + '): ' + resp.url.substr(0,120));

  }

  if (resp.bodySize) {
  
    bandwidth += resp.bodySize;
...
page.onResourceError = function (error) {
  ...
  if (error.errorCode !== 301) {
    ...
    console.debug('Error code: ' + error.errorCode + '. Description: ' + error.errorString);
...
// on page load, parse url location and make decisions 
page.onLoadFinished = function (status) {

  if (page.title === 'IPBlocked') {
  ...
  switch (loc) {
    ...
    default:
      console.debug('no handle for ' + loc);
      break;
...
// define callbacks to phantomjs from the in-page js.
page.onCallback = function (data) {
  ...
  switch(data.handle) {
    ...
    case "warn":
      console.log(colored("red", data.msg));
      break;

    default:
      console.debug('no handle for ' + data.handle);
      console.log(data.msg);
      break;
...
function getLocation() {
  ...
  return page.evaluate(function () {
    ...
    var loc = window.location.pathname,
        dir = loc.substring(loc.lastIndexOf('/') + 1, loc.length);
...
// handle for the landing page.
function handleLandingPage() {
  
  page.evaluate(function (usr, pwsd) {
  
      // assume we're on a page with a entry point to the login page
      window.location = $('a[href*="login"]').prop('href');
...
function handleLogin() {
  ...
  // use a post request instead of navigating, 
  // so we can react to postback messages.

  page.evaluate(function (usr, pwsd) {
    
    // assume we're on a page with a entry point to the login page
    var loginUrl = $('#frmBasicInfo').prop('action');
    
    $.post(loginUrl, "EmailAddress="+ usr +"&Password="+ pwsd, function (data) {
      
      // the user is valid, if we get a returnUrl, 
      // banned or non-existing otherwise
      if (data.returnUrl) { window.location = data.returnUrl; }
      else { 
      
        if (typeof window.callPhantom === 'function') {
      
          window.callPhantom({ handle: 'exit', msg: data.Message });
...
function handleHome () {
  ...
  page.evaluate(function () {
    ...
    (function checkVersion() {
      ...
      var currentVersion = 'mlNpjQDx8JDiEpZ_ZPvBADDKX5gF-7V11MKj16ZgOpk1',
        ...
        if (version !== currentVersion) {
          ...
          callHome({
            handle: 'warn',
            msg: 'Adplay version changed! this program needs an update!'
          });
    ...
    // wrapper for calling phantom from the page
    ...
    function timer () {
      ...
      if (timeleft <= 0) { clearInterval(window.displayTimeoutTimer); }
    ...
    function vEnded (){
      ...
      // open the video ended dialog
      videoEndedDialog.nextClick(); // click the dialog

      // wait for a response from the server
      window.sEndedTimer = setTimeout(sEnded, 3000 ); 
    ...
    function sEnded (){
      ...
      // have the server responded yet?
      if (!sessionEndedDialog.doneClick) { 
        
        window.sEndedTimer = setTimeout(sEnded, 1000);
        return;

      }      
      // end the session and return to the home screen
      sessionEndedDialog.doneClick();    

      // recursive step.
      window.initWhirlerTimer = setTimeout(initWhirler, 3000);

    }

    var captchaDetector = setInterval(function(){
      
      if (captchaDialog.onValidated) {

          callHome({ 
            handle: 'exit',
            msg: 'captcha has been shown! abort abort!' 
          });
      }
    ...
    function runner () {
      ...
      clearInterval(window.displayTimeoutTimer);
      clearTimeout(window.vEndedTimer);
      clearTimeout(window.sEndedTimer);
      clearTimeout(window.initWhirlerTimer);

      // find the number of ads left
      var watched = parseInt(document.getElementById('spnWatchedAds').innerText),
          total = parseInt(document.getElementById('spnTotalAds').innerText),
          ads = total - watched,
      ...
      // close the referral pop-up
      $('#popupReferrerClose').click();
        
      // manually set the verification values.
      try{
        
        campaignViewed = true;
        abm.updateCampaignIsViewed(true);
        abm.updateStartTime();
        updateTracking();
      ...
      // natural timeout..
      var duration = abm.getCampaignMediaDuration() * 1000,
          // add a small delay to obfuscate our precision
          randomEndTime = Math.random() * (2000 - 200) + 200;

      showVideoEndedDialog();

      callHome({
        handle: 'log', 
        msg: 'Starting ad ' + (total - ads) + '/' + total
      });      

      window.vEndedTimer = setTimeout(vEnded, duration + randomEndTime);
      timeleft = (Math.round((duration + randomEndTime) / 1000));
      window.displayTimeoutTimer = setInterval(timer, 1000);
    
    }

    function initWhirler () {
      exitFullwindowPlayer = closeAllDialogs; 
...
function handleUserData () {
  ...
  page.evaluate(function (){
    ...
    // select a random age between 16 - 86
    var age = Math.round(Math.random() * (86 - 16) + 16),
        // select a random zipcode
        postal = postalcodes.getRandom();

    $('#Age').val(age);
    $('#ZipCode').val(postal);
    $('#btnSavePostalCode').click();
  
    // give the page a 4s grace period, redirect otherwise   
    setTimeout(function(){
...
// begin here, so that AJAX request will be same-origin.
page.open('http://eovendoredirect.azurewebsites.net/Home');
